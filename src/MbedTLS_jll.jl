...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import MbedTLS_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("MbedTLS_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "MbedTLS_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"MbedTLS", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "MbedTLS_jll", "src", "MbedTLS_jll.jl"))
        @info("MbedTLS_ll dev'ed out to /depot/dev/MbedTLS_jll with pre-populated override directory")
    end
end
